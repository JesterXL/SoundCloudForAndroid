package com.dasflash.soundcloud.as3api
{
	...
	public class SoundcloudClient extends EventDispatcher
	{
		...
		{
			if(event.data == "Invalid OAuth Request")
			{
				//dispatchEvent(new SoundcloudFaultEvent(SoundcloudFaultEvent.REQUEST_TOKEN_FAULT, "Invalid OAuth Request"));
				return;
			}
        ...
        public function authorizeUserMobile(webView:StageWebView):void
        {
            //Debug.logHeader();
            //Debug.log("SoundcloudClient::authorizeUserMobile");
			// add request parameters
			var params:URLVariables = new URLVariables();
			params["oauth_token"] = requestToken.key;
			...
			// open url in StageWebView
            var parsedURL:String = authURL + "/?" + params.toString();

            //Debug.log("webView: " + webView + ", url: " + parsedURL);
